...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    try {
      ...
      const newSolution = await MasterService.create({ name });
    ...
    try {
      const solutions = await MasterService.find({});
    ...
    try {
      ...
      const updatedSolution = await MasterService.findByIdAndUpdate(
        id,
        { name },
        { new: true }
      );
    ...
    try {
      ...
      const deletedSolution = await MasterService.findByIdAndDelete(id);
      if (!deletedSolution) {
        return res
          .status(404)
          .json({ success: false, error: "Solution not found" });
