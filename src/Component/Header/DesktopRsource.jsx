...
//       {resourceopen && (
//         <div className="absolute left-0 mt-2 w-64 h-44   rounded-md bg-white shadow-lg z-10">
  ...
  return (
    ...
    >
      <button className="focus:outline-none hover:text-orange-500 flex items-center relative">
        Resources
        {resourceopen ? (
          <FaChevronUp className="ml-1 transition-transform duration-300" size={15} />
        ) : (
          <FaChevronDown className="ml-1 transition-transform duration-300" size={15} />
        )}
        <span className="absolute left-0 bottom-0 h-0.5 w-full transition-all duration-300 transform scale-x-0 hover:scale-x-100" />
      </button>
