...
const TechProducts = ({ productTech }) => {
  // console.log("here is all data productTech---->", productTech);
  return (
    <>
      <div>
        <div>
          <p
            ...
            dangerouslySetInnerHTML={{
              __html: productTech?.mainEditorHtmlDescription,
            }}
          />
        </div>
        {/* Card section */}
        <div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8 pt-5">
            {productTech?.productItem?.map((item, index) => (
              <div
                key={index}
