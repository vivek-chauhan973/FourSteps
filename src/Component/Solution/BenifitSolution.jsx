...
const BenifitSolution = ({ solutionBenifits }) => {
  // console.log("here is all my benifits data here-------->>>", solutionBenifits);
    ...
    <>
      <div className="">
        <h2
          ...
          dangerouslySetInnerHTML={{ __html: solutionBenifits?.description }}
        ></h2>
        ...
        <div className="block md:hidden text-center">
          <div className="border-l-[3px]   py-4 px-2 inline-block">
            <div className="text-heading text-4xl font-semibold">
              {solutionBenifits?.items[currentIndex]?.title}
            </div>
            <div className="text-gray-700 mt-2">
              {solutionBenifits?.items[currentIndex]?.description}
            </div>
          </div>
          <div className="flex justify-center mt-4 space-x-2">
            {solutionBenifits?.items?.map((_, index) => (
              <button
                key={index}
                className={`h-1 w-8 ${
        ...
        <div className="hidden md:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-3">
          {solutionBenifits?.items?.map((stat, index) => (
...
export default BenifitSolution;
