...
const fetchAllSuccessStories = async (id) => {
  const res = await fetch(`/api/solution/solution-overview?id=${id}`, {
    method: "GET",
  });
  return await res.json();
};

const SolutionOverview = ({ setActiveTab, blogData }) => {
  ...
  async function handleUpload() {
    ...
    try {
      const url = isUpdating
        ? `/api/solution/solution-overview/${editItemId}`
        : `/api/solution/solution-overview`;
  ...
  const handleSave = async () => {
    ...
    const res = await fetch(
      `/api/solution/solution-overview/overview?solution=${blogData?._id}`,
      {
        method: "POST",
        headers: {
    ...
    if (res?.ok) {
      ...
      alert(
        blogData?._id
          ...
          : "overview Data saved successfully"
      );
    } else {
  ...
  return (
    <>
      <div className="p-4 mb-5 rounded-md bg-white shadow-[0_0px_10px_-3px_rgba(0,0,0,0.3)] border-l-2 border-teal-600">
        <p className="text-base font-semibold mb-2">Solution Overview</p>
        <div className="p-4">
            ...
            <div className="border py-3 px-4">
              ...
              <div className="my-5">
                ...
                <div className="w-full">
                  <h3 className="font-semibold mb-2">Overview description</h3>
                  <QuillNoSSRWrapper
                    className="rounded h-48 mb-16"
                    theme="snow"
