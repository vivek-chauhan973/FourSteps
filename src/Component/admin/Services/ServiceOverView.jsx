...
const fetchAllSuccessStories = async (id) => {
  const res = await fetch(`/api/service/service-overview?id=${id}`, {
    method: "GET",
  });
  return await res.json();
};

const ServiceOverView = ({ setActiveTab, blogData }) => {
  ...
  async function handleUpload() {
    ...
    if (file && title) {
      ...
      formData.append("service", blogData?._id);
    }
    try {
      const url = isUpdating
        ? `/api/service/service-overview/${editItemId}`
        : `/api/service/service-overview`;
  ...
  return (
    <>
      <div className="p-4 mb-5 rounded-md bg-white shadow-[0_0px_10px_-3px_rgba(0,0,0,0.3)] border-l-2 border-teal-600">
        <p className="text-base font-semibold mb-2">Service Overview</p>
        <div className="p-4">
