  ...
  async function handleUpload() {
    ...
    try {
      const res = await fetch(
        `/api/technology/${blogData ? blogData?._id : "technology-hero"}`,
        {
      ...
      if (res?.ok) {
        router.push("/admin/technology/" + data1?.data?._id);
        setActiveTab("Tab2");
  ...
  return (
    <>
      <div className="p-4 mb-5 rounded-md bg-white shadow-[0_0px_10px_-3px_rgba(0,0,0,0.3)]  border-l-2 border-teal-600">
        <p className="text-base font-semibold mb-2">Technology Hero Section Detail</p>
        <div className="p-4">
