...
const fetchAllSuccessStories = async (id) => {
  const res = await fetch(`/api/technology/success1?id=${id}`, { method: "GET" });
  return await res.json();
};

const TechnologySuccessStories = ({ setActiveTab, blogData }) => {
  ...
  async function handleUpload() {
    ...
    try {
      const url = isUpdating
        ? `/api/technology/success1/${editItemId}`
        : `/api/technology/success1`;
  ...
  const deleteItem = async (id) => {
    const data = await fetch(`/api/technology/success1?id=${id}`, {
      method: "DELETE",
    });
    if (data?.ok) {
  ...
  const handleSaveAllData = async () => {
    ...
    const res = await fetch(
      `/api/technology/success1/success?technology=${blogData?._id}`,
      {
        method: "POST",
        headers: {
  ...
  return (
    <>
      <div className="p-4 mb-5 rounded-md bg-white shadow-[0_0px_10px_-3px_rgba(0,0,0,0.3)] border-l-2 border-teal-600">
        <p className="text-base font-semibold mb-2">Technology Success stories</p>
        <div className="p-4">
...
export default TechnologySuccessStories;
