...
const fetchAllSuccessStories = async (id) => {
  const res = await fetch(`/api/technology/services?id=${id}`, {
    method: "GET",
  });
  return await res.json();
};

const TechnologyServices = ({ setActiveTab, blogData }) => {
  ...
  async function handleUpload() {
    ...
    try {
      const url = isUpdating
        ? `/api/technology/services/${editItemId}`
  ...
  const deleteItem=async (id)=>{
    const res=await fetch(`/api/technology/services?id=${id}`,{method:"DELETE"});
    if(res?.ok){
      fetchAllSuccessStories(blogData?._id).then((res) => {
        setCurrentItems(res?.data || []);
        const data = res?.data?.map((item) => item?._id);
  ...
  const handleSave=async ()=>{
   ...
   const res=await fetch(`/api/technology/services/service?technology=${blogData?._id}`,{
    method:"POST",
    headers:{
