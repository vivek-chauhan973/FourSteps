...
const fetchAllSuccessStories = async (id) => {
  const res = await fetch(`/api/technology/products?id=${id}`, {
    method: "GET",
  });
  return await res.json();
};

const TechnologyProducts = ({ setActiveTab, blogData }) => {
  ...
  async function handleUpload() {
    ...
    try {
      const url = isUpdating
        ? `/api/technology/products/${editItemId}`
        : `/api/technology/products`;
  ...
  const deleteItem=async (id)=>{
    const res=await fetch(`/api/technology/products?id=${id}`,{method:"DELETE"});
    if(res?.ok){
      fetchAllSuccessStories(blogData?._id).then((res) => {
        setCurrentItems(res?.data || []);
        const data = res?.data?.map((item) => item?._id);
  ...
  const handleSave=async ()=>{
   ...
   const res=await fetch(`/api/technology/products/product?technology=${blogData?._id}`,{
    method:"POST",
    headers:{
