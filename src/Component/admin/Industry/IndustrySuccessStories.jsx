...
const fetchAllSuccessStories = async (id) => {
  const res = await fetch(`/api/industry/success?id=${id}`, { method: "GET" });
...
const IndustrySuccessStories = ({ setActiveTab, blogData }) => {
  ...
  async function handleUpload() {
    ...
    try {
      const res = await fetch(`/api/industry/success`, {
        method:"POST",
        body: formData,
      });
  ...
  const deleteItem = async (id) => {
    const data = await fetch(`/api/industry/success?id=${id}`, {
      method: "DELETE",
    });
    if (data?.ok) {
