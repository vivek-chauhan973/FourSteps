...
const IndustrySuccessStories = ({ setActiveTab, blogData }) => {
  ...
  const handleSaveAllData = () => {
    const allDataToSave = allItems.map((item) => ({
    ...
    console.log("Saving all data:", allDataToSave);

    fetch("/api/industry/saveAllData", {
      method: "POST",
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg ${
                  currentPage === index + 1
                    ...
                    : "bg-gray-200 text-gray-600 hover:bg-teal-200"
                }`}
                onClick={() => handleChangePage(index + 1)}
              >
                {index + 1}
