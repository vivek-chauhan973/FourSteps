...
const fetchAllSuccessStories = async (id) => {
  const res = await fetch(`/api/industry/industry-overview?id=${id}`, {
    method: "GET",
  });
  return await res.json();
};

const IndustryOverview = ({ setActiveTab, blogData }) => {
  ...
  async function handleUpload() {
    ...
    try {
      const url = isUpdating
        ? `/api/industry/industry-overview/${editItemId}`
        : `/api/industry/industry-overview`;
  ...
  const handleSave = async () => {
    ...
    const res = await fetch(
      `/api/industry/industry-overview/overview?industry=${blogData?._id}`,
      {
        method: "POST",
        headers: {
  ...
  return (
    <>
      <div className="p-4 mb-5 rounded-md bg-white shadow-[0_0px_10px_-3px_rgba(0,0,0,0.3)] border-l-2 border-teal-600">
        <p className="text-base font-semibold mb-2">Iindustry Overview</p>
        <div className="p-4">
